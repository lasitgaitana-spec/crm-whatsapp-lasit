...
export default function BulkMessagesProgram() {
  ...
  const loadFlowById = async (id) => {
    try {
      const res = await fetch(`/api/bulk/flows/${id}`)
      const data = await res.json()
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(async () => {
      try {
        ...
        const res = await fetch(`/api/bulk/flows/${idNum}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ connections }) })
        const data = await res.json()
  ...
  const manualSaveNow = async () => {
    try {
      ...
      const connections = JSON.stringify({ steps })
      const res = await fetch(`/api/bulk/flows/${idNum}`, {
        method: 'PUT',
    ...
    const loadFields = async () => {
      try {
        ...
        if (data?.ok && Array.isArray(data.items)) {
          // Usar solo los campos configurados y evitar duplicados por nombre
          const raw = data.items.map(f => ({ id: f.id, name: f.name }))
          const seen = new Set()
          const unique = []
          for (const it of raw) {
    ...
    <Stack spacing={3}>
      ...
      <Paper sx={{ p: 2 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
          ...
          <Button variant="outlined" onClick={() => navigate('/campaigns/bulk-messages')}>Volver a Paso 1</Button>
        </Box>
        ...
        <Box sx={{ mt: 2 }}>
          ...
          {saveStatus === 'saved' && (
            <Box sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}>
              <Chip icon={<CheckCircleOutlineIcon />} color="primary" label="Cambios Guardados" />
            </Box>
          )}
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: 3 }}>
              ...
              {activeForm === 'texto' && (
                <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 1.5, p: 1.5, bgcolor: 'background.default' }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    ...
                    <Button variant="contained" color="primary" onClick={addTextStep}>Guardar</Button>
                  </Box>
              ...
              {activeForm === 'imagen' && (
                <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 1.5, p: 1.5, bgcolor: 'background.default', mt: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    ...
                    <Button variant="contained" color="primary" onClick={addImageStep} disabled={imageUploading}>Guardar</Button>
                  </Box>
                </Box>
              )}
              ...
              {activeForm === 'archivo' && (
                <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 1.5, p: 1.5, bgcolor: 'background.default', mt: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    ...
                    <Button variant="contained" color="primary" onClick={addFileStep} disabled={fileUploading}>Guardar</Button>
                  </Box>
                </Box>
              )}
              ...
              {activeForm === 'video' && (
                <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 1.5, p: 1.5, bgcolor: 'background.default', mt: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    ...
                    <Button variant="contained" color="primary" onClick={addVideoStep} disabled={videoUploading}>Guardar</Button>
                  </Box>
                </Box>
              )}
              ...
              {activeForm === 'audio' && (
                <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 1.5, p: 1.5, bgcolor: 'background.default', mt: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    ...
                    <Button variant="contained" color="primary" onClick={addAudioStep} disabled={audioUploading}>Guardar</Button>
                  </Box>
                </Box>
              )}
              ...
              {activeForm === 'contacto' && (
                <Box sx={{ mt: 2, border: '1px solid #fee', bgcolor: '#fff5f5', borderRadius: 1.5, p: 1.5 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    ...
                    <Button variant="contained" color="primary" onClick={addContactStep}>Guardar</Button>
                  </Box>
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1.5 }}>
                {[
                  ...
                  <Paper
                    ...
                    sx={{ gridColumn: btn.type === 'probar' ? 'span 2' : 'auto', p: 2, borderRadius: 2, cursor: 'pointer', border: '1px solid', borderColor: 'divider', bgcolor: 'background.paper', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1, transition: 'all .15s ease-in-out', '&:hover': { borderColor: 'primary.light', boxShadow: 1 } }}
                  >
                    <btn.Icon sx={{ color: 'primary.main' }} />
              ...
              {activeForm === 'retraso' && (
                <Box sx={{ mt: 2, border: '1px solid #fee', bgcolor: '#fff5f5', borderRadius: 1.5, p: 1.5 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    ...
                    <Button variant="contained" color="primary" onClick={addDelayStep}>Guardar</Button>
                  </Box>
                </Box>
              )}
              {activeForm === 'autooff' && (
                <Box sx={{ mt: 2, border: '1px solid #fee', bgcolor: '#fff5f5', borderRadius: 1.5, p: 1.5 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    ...
                    <Button variant="contained" color="primary" onClick={addAutoOffStep}>Guardar</Button>
                  </Box>
                </Box>
              )}
            </Box>
            ...
            <Box>
              ...
              <Paper variant="outlined" sx={{ p: 2 }}>
                    ...
                    <Box key={s.id} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: '1px dashed #e0e0e0', borderRadius: 1, p: 1 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        ...
                        {s.type === 'audio' && s.payload?.url && (
                          <Chip size="small" label={(s.payload.url.split('/').pop() || 'audio')} onClick={()=>openPreview(s.payload.url)} sx={{ ml: 1, cursor: 'pointer' }} />
                        )}
                        {s.type === 'contacto' && (
                ...
                {/* Removed lower "Cambios Guardados" message to avoid duplication */}
              </Paper>
            </Box>
          </Box>
        </Box>
      ...
      <Dialog open={deleteStepDialogOpen} onClose={closeDeleteStepDialog}>
        ...
        <DialogContent>
          <Typography>¿Seguro que deseas eliminar este paso?</Typography>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={testOpen} onClose={closeTestModal} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers>
          <Stack spacing={2}>
            <FormControl fullWidth>
              ...
              <Select labelId="platform-label" label="Plataforma" value={testPlatform} onChange={(e)=>handleTestPlatformChange(e.target.value)}>
            ...
            {testLoading ? (
              ...
              <List dense sx={{ maxHeight: 320, overflow: 'auto', border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
                ...
                {testItems.map(it => (
                  <ListItemButton key={it.key} onClick={()=>toggleSelectTestItem(it.key)} selected={testSelected.includes(it.key)}>
                    <ListItemText primary={it.label} secondary={it.subLabel} />
                    <Checkbox checked={testSelected.includes(it.key)} />
                  </ListItemButton>
                ))}
              </List>
            )}
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={sendTest} disabled={testSelected.length===0 || testSending}>{testSending ? 'Enviando…' : 'Enviar'}</Button>
        </DialogActions>
      </Dialog>
    </Stack>
    </>
