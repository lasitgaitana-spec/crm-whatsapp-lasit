...
export default function SettingsOpenAIApiKey() {
  ...
  const save = async () => {
    try {
      ...
      setError('')
      setSuccess('')
      const r = await fetch('/api/settings/openai/apikey', {
        method: 'POST',
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="h5">OpenAI – API Key</Typography>
      <Paper sx={{ p: 2 }}>
        <Stack spacing={2}>
          ...
          <TextField label="Nueva API Key de OpenAI" type="password" value={apiKey} onChange={(e)=>setApiKey(e.target.value)} fullWidth />
