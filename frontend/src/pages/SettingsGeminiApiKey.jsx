...
export default function SettingsGeminiApiKey() {
  ...
  const save = async () => {
    try {
      ...
      setError('')
      setSuccess('')
      const r = await fetch('/api/settings/gemini/apikey', {
        method: 'POST',
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="h5">Gemini – API Key</Typography>
      <Paper sx={{ p: 2 }}>
        <Stack spacing={2}>
          ...
          <TextField label="Nueva API Key de Gemini" type="password" value={apiKey} onChange={(e)=>setApiKey(e.target.value)} fullWidth />
